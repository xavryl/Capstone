...
export default function Chat() {
  ...
  useEffect(() => {
    const initChatFromNav = async () => {
      if (location.state?.sellerId && currentUser) {
        const { sellerId, cropTitle, sellerName, reportReason, reportDetails, offerAmount, originalPrice } = location.state;
  ...
  const handleSelectChat = async (user) => {
    ...
    try { 
        ...
        // Ignore error if chat doesn't exist yet (first time chat)
...
function FullPageChatWindow({ activeChat, currentUser, onReport }) {
  ...
  const handleOfferAction = async (msg, action) => {
      ...
      const text = action === 'accept' 
        ? `✅ Offer Accepted! Deal is now in progress.` 
        : `❌ Offer Declined.`;
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-50">
        {messages.map(msg => {
          ...
          // --- 3. OFFER CARD (With Counter Feature) ---
          if (msg.isOffer) {
             return (
                    ...
                    <div className={`p-4 rounded-xl shadow-sm border ${isMe ? 'bg-yellow-50 border-yellow-200' : 'bg-white border-gray-200'} max-w-[80%]`}>
                        ...
                        {msg.offerStatus && msg.offerStatus !== 'pending' && (
                            <div className={`text-xs font-bold text-center py-1 rounded ${
                                ...
                                'text-blue-600 bg-blue-100' // Countered
                            }`}>
          ...
          // --- 4. TRANSACTION CARD (Legacy Support) ---
