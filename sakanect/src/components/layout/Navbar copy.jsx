...
import { Sprout, MessageCircle, LogOut, Bell, CheckCircle, ChevronDown, Globe, Menu, User } from 'lucide-react';
...
export default function Navbar() {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.docs.forEach(doc => {
        ...
        if (data.participants?.includes(user.id) && data.unreadBy?.includes(user.id)) {
          ...
          const senderName = data.users?.[senderId]?.name || data.users?.[senderId]?.username || "Someone";
          unreadList.push({ id: doc.id, senderName, message: data.lastMessage, time: data.lastMessageTime });
        }
      });
      unreadList.sort((a, b) => b.time?.seconds - a.time?.seconds);
  ...
  return (
    <>
      ...
      {/* Added 'no-scrollbar' and 'overflow-y-auto' to allow scrolling without the visual bar */}
        ...
        {user && (
            <div className="hidden md:flex flex-col w-full p-4 border-t border-gray-100 bg-gray-50 mt-auto">
                <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 rounded-full bg-saka-green text-white flex items-center justify-center font-bold text-lg">
      ...
      {user && (
          <div className="fixed top-4 right-6 z-50 flex items-center gap-3">
            ...
            {/* LANGUAGE DROPDOWN (Replaced Buttons) */}
            ...
            <div className="relative" ref={notifRef}>
                <button 
                  ...
                  className="w-10 h-10 bg-white hover:bg-gray-50 rounded-full shadow-md border border-gray-100 flex items-center justify-center text-gray-600 transition"
                >
                  <Bell size={20} />
